...
const cards = [
  {
    ...
    text: "Create content about growing your IG audience? Use concise hooks and formats that convert.",
    icon: <PiMonitorPlay />,
    accent: "#b48cff",
  },
  {
    title: "Fitness",
    text: "Workout, nutrition, or challenges—scripts tuned for watch time and saves.",
    icon: <CiDumbbell />,
    accent: "#80d3ff",
  },
  {
    title: "Fashion",
    text: "Lookbooks, styling tips, and hauls with retention-first storylines.",
    icon: <PiDress />,
    accent: "#ffb3d1",
  },
  {
    title: "Glow-up tips",
    text: "Skincare & beauty: clean hooks and step-by-steps your audience will follow.",
    icon: <TbBrush />,
    accent: "#9fe2bf",
  },
  {
    title: "Dark psychology",
    text: "Cover persuasion topics responsibly with high-engagement structures.",
    icon: <PiMaskHappy />,
    accent: "#d6d6d6",
  },
  {
    title: "Tech",
    text: "Explainers, reviews, and AI tools—scripts that simplify complexity.",
    icon: <GrTechnology />,
    accent: "#9ad0ff",
  },
  {
    title: "Digital products",
    text: "Selling courses, presets, or templates? Lift CTR and sales with proven angles.",
...
const Facilities = () => {
  ...
  useEffect(() => {
    ...
    if (!hasTriggeredRef.current && alreadyVisible) {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        if ((entry.isIntersecting || entry.intersectionRatio > 0.12) && !hasTriggeredRef.current) {
          hasTriggeredRef.current = true;
          setRevealed(true);
          observer.disconnect();
        }
        ...
        <div className="cards" role="list" aria-label="Example niches">
          {cards.map((card, index) => (
            <article
              ...
              tabIndex={-1}
              className="card"
              aria-label={`${card.title} — example niche`}
              style={{
                "--accent": card.accent,
            ...
            >
              <div className="card__icon" aria-hidden="true">
                <span className="card__iconCircle">{card.icon}</span>
              </div>
